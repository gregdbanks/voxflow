...
Haiku 4.5 responses (the spec'd model). Run `npx tsx scripts/demo-real-bedrock.ts`
with AWS credentials exported to reproduce.
...
version `"So I was thinking we should probably ship the feature tomorrow."` —
fillers removed (`um`, `like`, `you know`), sentence capitalized, period added.
This exact string came back from Bedrock's Haiku 4.5 on 2026-04-23 (see the
`chat` entry in the context gallery below).
...
VS Code (code context). Three identifiers auto-CamelCased: `fetch user profile` →
`fetchUserProfile`, `user ID` → `userID`, and the return type `user profile` →
`UserProfile`. That's the `code` system prompt's "preserve technical terms,
CamelCase, identifiers" instruction doing actual work — and the kind of
transformation a regex-based cleaner couldn't touch. 256 / 24 tokens.
...
Totals across all four: **913 input + 93 output tokens ≈ \$0.00138** at Haiku
4.5 pricing (\~\$1/MTok in, \~\$5/MTok out on Bedrock). That's ≈ **\$0.00035 per
cleanup** — well under the M6 spec's < \$0.001 target.

## Model override

The service's default `modelId` is `us.anthropic.claude-haiku-4-5-20251001-v1:0`
per issue #6. To test against a different inference profile without touching
source, use the demo script's env var:
...
Requires an active AWS Marketplace subscription for whichever model you pick.
